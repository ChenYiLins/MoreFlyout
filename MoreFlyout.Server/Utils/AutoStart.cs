...
internal class AutoStart
{
    ...
    public static bool SetAutoStart(bool enabled)
    {
        ...
        if (exePath is null)
        {
            Logger.Error("Failed to get the executable path for auto-start configuration");
            return false;
        }
        ...
        try
        {
            ...
            if (key is null)
            {
                Logger.Error("Failed to open registry key for auto-start configuration");
                return false;
            }
            ...
            if (enabled)
            {
                ...
                Logger.Info($"Enabled auto-start for MoreFlyout with executable path: {exePath}");
            }
            else
            {
                if (key.GetValue("MoreFlyout") is not null)
                {
                    ...
                    Logger.Info("Disabled auto-start for MoreFlyout");
                }
            }
            return true;
        }
        ...
        {
            Logger.Error(ex, $"Failed to update auto-start configuration in the registry: {ex.Message}");
            return false;
        }
    }
    ...
    public static bool CheckAutoStart()
    {
        ...
        {
            Logger.Error(ex, $"Failed to check auto-start configuration in the registry: {ex.Message}");
            return false;
        }
    }
}
